      ...
      <div className={cardCls + ' !p-0 overflow-hidden'}>
        ...
        {showCreate && (
          <div className="px-4 py-4 bg-calm-50 dark:bg-slate-800/50 border-b border-calm-200 dark:border-slate-700 space-y-3">
            ...
            <div className="flex items-center gap-2">
              <button onClick={createApiKey} disabled={!newKeyName.trim() || creating} className={primaryBtnCls}>
                {creating ? 'Creating...' : 'Generate Key'}
              </button>
              <button onClick={() => setShowCreate(false)} className={secondaryBtnCls}>
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className={!loading && apiKeys.length > 0 ? 'animate-stagger' : ''}>
              {loading ? (
                <tr>
                ...
                apiKeys.map((k) => (
                  <tr
                    key={k.id}
                    className="border-b border-calm-100 dark:border-slate-700 hover:bg-calm-50 dark:hover:bg-slate-800/50 transition-colors"
                  >
      ...
      <div className={cardCls}>
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-slate-900 dark:text-white">
            Connecting with Microsoft Power Automate
          </h3>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <ol className="list-decimal list-inside space-y-2 text-sm text-slate-600 dark:text-slate-400">
              <li>
                Go to <strong>make.powerautomate.com</strong> and create a new <strong>Scheduled cloud flow</strong> (e.g. run daily or weekly).
              </li>
              <li>
                Add an <strong>HTTP</strong> action with these settings:
              </li>
            </ol>
            <div className="mt-2 bg-slate-50 dark:bg-slate-800 rounded-lg p-4 border border-calm-200 dark:border-slate-600 space-y-2">
              ...
              <div className="flex items-start gap-3 text-xs">
                ...
                <code className="text-slate-800 dark:text-slate-200 break-all">
                  {typeof window !== 'undefined' ? window.location.origin : ''}/api/integrations/reports
                </code>
              </div>
              <div className="flex items-start gap-3 text-xs">
          ...
          <div>
            ...
            <ol className="list-decimal list-inside space-y-2 text-sm text-slate-600 dark:text-slate-400">
              <li>
                After the HTTP action, add a <strong>Parse JSON</strong> action using the HTTP response body.
              </li>
              <li>
                Use an <strong>Apply to each</strong> loop to iterate over the training/survey/library arrays.
              </li>
              <li>
                Add a <strong>Dataverse — Add a new row</strong> action to create records in your Dynamics 365 entity.
              </li>
              <li>
                Map JSON fields (e.g. <code className="bg-slate-100 dark:bg-slate-700 px-1 py-0.5 rounded text-xs">organisationName</code>, <code className="bg-slate-100 dark:bg-slate-700 px-1 py-0.5 rounded text-xs">completionRate</code>) to Dynamics columns.
              </li>
            </ol>
          </div>
