...
const ROLE_STYLES: Record<string, string> = {
  ...
  CHARITY_EMPLOYEE: 'bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-300',
}
...
const ROLE_LABELS: Record<string, string> = {
  ...
  CHARITY_EMPLOYEE: 'Charity Staff',
}
...
export default function CharitySessionDetailPage() {
  ...
  const DELETE_CONFIRM_PHRASE = 'Yes I want to delete this'
  ...
  const fetchSession = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/super-admin/sessions/${sessionId}`)
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    // Fetch organisations for name lookup (best-effort — may 403 for non-SUPER_ADMIN)
    fetch('/api/super-admin/organisations')
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (d) {
          const list: Organisation[] = Array.isArray(d) ? d : (d.organisations ?? d.orgs ?? [])
          setOrgs(list)
        }
      })
      .catch(() => {})

    // Check if charity meeting config is set up
  ...
  function getOrgName(organisationId: string | null): string {
    if (!organisationId) return '—'
    const org = orgs.find((o) => o.id === organisationId)
    return org ? org.name : organisationId.slice(0, 8) + '…'
  }
  ...
  async function handleSaveAttendance() {
    ...
    try {
      ...
      const res = await fetch(`/api/super-admin/sessions/${sessionId}/attendance`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function generateMeeting() {
    setGenerating(true)
    try {
      const res = await fetch(`/api/super-admin/sessions/${sessionId}/generate-meeting`, {
        method: 'POST',
      })
      if (res.ok) {
  ...
  async function handleDelete() {
    setDeleting(true)
    try {
      const res = await fetch(`/api/super-admin/sessions/${sessionId}`, { method: 'DELETE' })
      if (res.ok) {
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-page-enter">
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-red-200 dark:border-red-900/50 p-6 shadow-xl max-w-md w-full mx-4 space-y-4">
            <h3 className="text-base font-bold text-red-600 dark:text-red-400 flex items-center gap-2">
              <Trash2 className="h-5 w-5" />
              Delete Session
            </h3>
            <p className="text-sm text-slate-600 dark:text-slate-300">
              This will permanently delete <span className="font-semibold">{data.title}</span> and all attendance records. This cannot be undone.
            </p>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Type <span className="font-mono font-bold text-red-600 dark:text-red-400">{DELETE_CONFIRM_PHRASE}</span> to confirm:
            </p>
            <input
              type="text"
              ...
              className="w-full px-3 py-2 rounded-lg border border-calm-200 dark:border-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-red-300 bg-white dark:bg-slate-700 dark:text-slate-100"
            />
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-calm-200 dark:border-slate-700 p-6 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="px-3 py-2 rounded-lg border border-calm-200 dark:border-slate-600 text-sm bg-calm-50 dark:bg-slate-700 text-slate-700 dark:text-slate-300">
              {data.host.name ?? 'Unnamed'} ({data.host.email})
            </div>
          </div>
        </div>
      ...
      {/* Section 4: Attendees & Attendance */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-calm-200 dark:border-slate-700 p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-calm-200 dark:border-slate-700">
                  ...
                  <th className="text-left px-3 py-2 font-semibold text-slate-600 dark:text-slate-400">
                ...
                {data.attendees.map((a) => (
                  <tr key={a.id} className="border-b border-calm-100 dark:border-slate-700/50">
                    ...
                    <td className="px-3 py-2.5 text-slate-500 dark:text-slate-400 text-xs">
                      {getOrgName(a.user.organisationId)}
                    </td>
