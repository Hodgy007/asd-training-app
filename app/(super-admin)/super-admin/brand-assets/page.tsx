...
export default function BrandAssetsPage() {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      // Reuse the existing library upload-url issuer — generic Blob token
      // gate, no library-specific logic.
        ...
        <form onSubmit={handleUpload} className="card space-y-4">
          ...
          <div>
            ...
            <input
              type="file"
              className="input w-full"
              accept="image/*,application/pdf,text/plain,text/markdown,.docx"
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={formSubmitting || !formFile || !formTitle.trim()}
              className="btn-primary disabled:opacity-50"
            >
              {formSubmitting ? 'Uploading…' : 'Upload asset'}
            </button>
          </div>
        </form>
      )}
