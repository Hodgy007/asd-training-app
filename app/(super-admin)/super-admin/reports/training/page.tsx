      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Cross-organisation module completion overview.
          </p>
        </div>
        <button
          onClick={fetchReport}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-calm-200 bg-calm-50">
                <th className="text-left px-4 py-3 font-semibold text-slate-600 whitespace-nowrap">Organisation</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-600 whitespace-nowrap">Users</th>
                {allModuleIds.map((id) => (
                  <th
                    key={id}
            ...
            <tbody className={!loading && !error && data.length > 0 ? 'animate-stagger' : ''}>
              {loading ? (
                <tr>
                  <td colSpan={2 + allModuleIds.length} className="px-4 py-12 text-center text-slate-400">
                ...
                <tr>
                  <td colSpan={2 + allModuleIds.length} className="px-4 py-10 text-center text-red-500">
                    {error}
                  </td>
                </tr>
              ) : data.length === 0 ? (
                ...
                data.map((org) => (
                  <tr key={org.orgId} className="border-b border-calm-100 hover:bg-calm-50 transition-colors">
                    ...
                    {allModuleIds.map((moduleId) => {
                      ...
                      return (
                        <td key={moduleId} className="px-3 py-3 text-center">
                          {stat ? <PctCell stat={stat} /> : <span className="text-slate-300">&mdash;</span>}
                        </td>
                      )
                    })}
                  </tr>
