    ...
    <div className="min-h-screen bg-orange-50 dark:bg-slate-900 flex items-center justify-center p-4 animate-page-enter">
      <div className="w-full max-w-md">
        ...
        <div className="card border-t-4 border-t-warm-500">
          ...
          {registeredOk && (
            <div className="flex items-start gap-2 bg-sage-50 border border-sage-200 rounded-xl p-3 mb-5">
              <AlertCircle className="h-4 w-4 text-sage-600 mt-0.5 flex-shrink-0" />
              <p className="text-sm text-sage-800">
                Your account is ready. Sign in below with the email and password you just chose.
              </p>
            </div>
          )}
          ...
          {ssoOrg?.sso === true && (ssoOrg.type !== 'charity' || ssoOrg.enforced) ? (
            /* Full-screen SSO branch (org SSO or enforced charity SSO) */
            <div className="space-y-4 text-center">
              ...
              <div>
                <p className="text-sm text-slate-500 mb-4">
                  {ssoOrg.type === 'charity'
                    ? 'Your organisation uses Charity Single Sign-On'
                    : 'Your organisation uses Enterprise Single Sign-On'}
                </p>
                <button
                  type="button"
            ...
            <>
              {/* Alternate sign-in buttons (charity SAML SSO + OAuth) — appear above password form */}
              {(() => {
                const showCharity = ssoOrg?.sso === true && ssoOrg.type === 'charity' && !ssoOrg.enforced
                const showGoogle = oauthProviders.google
                const showMicrosoft = oauthProviders.microsoft
                if (!showCharity && !showGoogle && !showMicrosoft) return null
                return (
          ...
          <div className="mt-6 pt-5 border-t border-calm-200 dark:border-slate-700 text-center space-y-2">
            <p className="text-sm">
              <Link href="/register" className="text-warm-600 hover:text-warm-700 dark:text-warm-400 font-medium">
