...
export default function CreateSessionPage() {
  ...
  // Fetch users and meeting config on mount
  useEffect(() => {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-calm-200 dark:border-slate-700 p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Meeting Link</label>
              <input
                type="url"
                value={meetingUrl}
        ...
        {/* Attendee Selection */}
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-calm-200 dark:border-slate-700 p-6 shadow-sm space-y-4">
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">Attendee Selection</h2>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={clsx(
                ...
                inviteAll ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-600'
              )}
            >
              <span
                className={clsx(
            ...
            <span className="text-sm text-slate-700 dark:text-slate-300 font-medium">Invite all members</span>
          </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {orgRoles.map((role) => (
                    ...
                    >
                      {selectedRoles.includes(role) && <CheckCircle className="inline h-3 w-3 mr-1" />}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 rounded-lg border border-calm-200 dark:border-slate-600 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300 bg-white dark:bg-slate-700 dark:text-slate-100"
                ...
                {selectedUsers.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {selectedUsers.map((u) => (
                      <span
                        key={u.id}
                        className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 text-xs font-medium"
                      >
                        {u.name ?? u.email}
                        <button
                          type="button"
