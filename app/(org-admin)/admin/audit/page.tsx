...
export default function OrgAdminAuditPage() {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 max-w-2xl">
            Every read, create, update, delete, AI-generation and export action recorded against
            a child in your organisation. Use this to answer parental subject-access requests.
          </p>
        </div>
        <button
          onClick={fetchData}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-calm-200 dark:border-slate-700 bg-calm-50 dark:bg-slate-800">
                ...
                <th className="text-left px-4 py-3 font-semibold text-slate-600 dark:text-slate-400">Actor</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-600 dark:text-slate-400">Child</th>
                <th className="text-left px-4 py-3 font-semibold text-slate-600 dark:text-slate-400 hidden xl:table-cell">IP</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                data.rows.map((r) => (
                  <tr key={r.id} className="border-b border-calm-100 dark:border-slate-800 hover:bg-calm-50 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-slate-800 dark:text-slate-200">{r.actor.name ?? '—'}</p>
                      <p className="text-xs text-slate-400">{r.actor.email ?? r.actor.id}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-medium text-slate-800 dark:text-slate-200">{r.child.name}</p>
