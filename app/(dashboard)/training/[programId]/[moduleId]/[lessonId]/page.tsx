...
export default function ProgramLessonPage({ params: rawParams }: LessonPageProps) {
  ...
  // Narration for the "Read the lesson text aloud" player at the bottom.
  // Intentionally excludes interactive block content — each block has its
  // own player at the top, and for carousels each slide has its own player
  // inside the carousel. This player covers the prose between blocks only,
  // so it no longer duplicates what the in-block readers already speak.
  //
  // Capped at the TTS API's 8000-char limit. For very long lessons this
  // means the bottom player narrates the opening portion; block-level
  // players cover the rest.
  const TTS_MAX_CHARS = 7900
  const lessonProseText = extractLessonProseTtsText(lesson.content, blocks)
  const wholeLessonTtsText = lessonProseText.length > TTS_MAX_CHARS
    ? lessonProseText.slice(0, TTS_MAX_CHARS)
    : lessonProseText
            ...
            if (segment.type === 'html') {
              ...
              return (
                <div key={`html-${idx}`} className="mb-4">
                  ...
                  <div
                    className="prose-lesson space-y-2 text-slate-700 dark:text-slate-300 overflow-x-hidden break-words [&_h1]:font-bold [&_h1]:text-xl [&_h1]:text-slate-900 [&_h1]:dark:text-slate-100 [&_h1]:mt-10 [&_h1]:mb-3 [&_h2]:font-semibold [&_h2]:text-lg [&_h2]:text-slate-900 [&_h2]:dark:text-slate-100 [&_h2]:mt-10 [&_h2]:mb-3 [&_h3]:font-semibold [&_h3]:text-base [&_h3]:text-slate-900 [&_h3]:dark:text-slate-100 [&_h3]:mt-8 [&_h3]:mb-2 [&_p]:mb-3 [&_p]:leading-relaxed [&_ul]:list-disc [&_ul]:ml-4 [&_ul]:sm:ml-6 [&_ul]:mb-4 [&_ol]:list-decimal [&_ol]:ml-4 [&_ol]:sm:ml-6 [&_ol]:mb-4 [&_li]:mb-1.5 [&_li]:leading-relaxed [&_strong]:text-slate-900 [&_strong]:dark:text-slate-100 [&_strong]:font-semibold [&_em]:italic [&_img]:max-w-full [&_img]:h-auto [&_img]:rounded-lg [&_table]:w-full [&_table]:overflow-x-auto [&_table]:block [&_table]:text-sm [&_pre]:overflow-x-auto [&_pre]:max-w-full [&_blockquote]:border-l-4 [&_blockquote]:border-primary-300 [&_blockquote]:pl-4 [&_blockquote]:italic [&_blockquote]:text-slate-600 [&_blockquote]:dark:text-slate-400 [&_blockquote]:my-4"
          ...
          {wholeLessonTtsText && (
            <div className="mt-6 pt-4 border-t border-calm-200 dark:border-slate-700">
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-2">
                Read the lesson text aloud
              </p>
              <TtsAudioPlayer
                text={wholeLessonTtsText}
                ariaLabel="Read the lesson text aloud"
              />
            </div>
          )}
        </div>
