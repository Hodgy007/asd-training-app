...
- `Organisation` -- multi-tenant org with `allowedProgramIds`, `allowedRoles`, `slug`, contact details (`contactName`, `contactEmail`, `contactPhone`), address fields (`addressLine1`, `addressLine2`, `city`, `county`, `postcode`, `country`), and hierarchy fields (`parentOrgId`, `isParentOrg`, `inheritSettings`, self-referencing `parentOrg`/`childOrgs` via "OrgHierarchy" relation)
