...
- **Lawful basis (Art. 6):** Configurable per organisation — most commonly `CONSENT` (Art. 6(1)(a)); Local Authorities may elect `PUBLIC_TASK` (Art. 6(1)(e)). The value is recorded on `Organisation.observationLawfulBasis` and is set by the super-admin in the organisation settings page.
...
| R4 | No evidence of who accessed a child's record | High | High | **High** | `ObservationAccessLog` records every create / update / delete / AI-generate / export with actor, child, IP, timestamp. Immutable by design — no UPDATE or DELETE endpoint. Viewable by the controller org-admin (`/admin/audit`, scoped to their own org) and by the caregiver themselves (`/activity`). AaA staff as processor have **no routine access** — minimises processor-side visibility into controller data. | Low | Med | Low |
